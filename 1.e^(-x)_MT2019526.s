     AREA     e, CODE, READONLY
     EXPORT __main
	 ENTRY 
__main  FUNCTION		         

      ;Calculating the value of e^(-x) upto infinite(or finite) terms
	  ;e^(-x) = 1 - x + ((x^2)/2) - ((x^3)/3!) + ((x^4)/4!) - ((x^5)/5!) +.....
	  ;Therefore, for the taken example: e^(-0.5) = 0.60653
...
LOOP4 VCMP.F32 s1,s5                   ;Comparing between the upper limit and the loop variable respectively
	  ...
	  VNEG.F32 s11,s10                 ;s11 is finally loaded with the value of e^(-x)
...
LOOP3 VCMP.F32 s13, s12
	  ...
	  END
									   ...
									   ;s12 represents the number of iterations done (or the terms upto which the calculation is done)